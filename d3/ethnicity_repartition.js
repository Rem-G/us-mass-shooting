function filterByEthnicity(data){
    ...
    ethnicities.forEach(eth => {
        eth_n = 0;
        data.forEach(shot => {
            if (shot.race.toLowerCase() == eth){
                eth_n += 1;
            }
        });

        final_data.push({'ethnicity': eth, 'number': eth_n});
    });
...
function draw_ethnicity(json){
    let data = filterByEthnicity(json);
